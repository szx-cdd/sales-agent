...
if not settings.kimi_api_key:
    raise RuntimeError("KIMI_API_KEY 环境变量未设置，请在 Railway 的 Variables 中添加")
...
async def quick_analyze(
    industry: str = Form("其他"),
    customer_info: str = Form(""),
    chat_history: str = Form(""),
    analysis_type: str = Form("full")
):
    try:
        if not customer_info.strip() and not chat_history.strip():
            return JSONResponse({
                "success": False, 
                "error": "请提供客户信息或聊天记录"
            }, status_code=400)
        ...
        if analysis_type == "profile":
            prompt = CUSTOMER_PROFILE_PROMPT.format(
                industry=industry,
                customer_info=customer_info,
                chat_history=chat_history or "暂无详细聊天记录"
            )
        elif analysis_type == "full":
            prompt = FULL_ANALYSIS_PROMPT.format(
                industry=industry,
...
async def upload_files(files: List[UploadFile] = File(None)):
    try:
        if not files:
            return JSONResponse({"success": False, "error": "没有上传文件"}, status_code=400)
        ...
        if not all_content:
            return JSONResponse({"success": False, "error": "没有可处理的文件内容"}, status_code=400)
